...
available by Stocker (n.d.) and GECO, University of Bern on GitHub. The
model will be trained using three different forms of cross-validation
(CV): random CV, spatial CV, and environmental CV.
...
trained and validated on folds between which the range of the predictor
values differs significantly. This is not the case for the folds of a
...
two hyper-parameters are set manually (mtry = 3 and min.nod.size = 12),
...
are very similar for each CV fold, shown in the table below.

```{r train random forest model, message = FALSE}
...
conditions than the geographic folds. Yet, this does not seem to be the case when comparing the distributions of the numeric predictor variables for the environmental clusters and the geographic clusters: 

```{r distribution of environmental conditions}

# create long dataframe for plotting
dfs_long <- dfs |>
  pivot_longer(
    cols = c("elv", "mat", "map", "ndep", "mai"),
    names_to = "predictor",
    values_to = "val"
  ) 

# plot density of predictors for each cluster
p1 <- ggplot(data = dfs_long, aes(x = val, color = cluster)) +
  geom_density() +
  facet_wrap(~predictor, ncol = 5, scales = "free") +
  theme_classic() +
  scale_color_brewer(palette = "Set1") +
  theme(axis.text.x = element_blank()) # remove x ticks 


p2 <- ggplot(data = dfs_long, aes(x = val, color = env_cluster)) +
  geom_density() +
  facet_wrap(~predictor, ncol = 5, scales = "free") +
  theme_classic() +
  scale_color_brewer(palette = "Accent") +
...
In order to understand the reasons for the significantly better model metrics of the environmental cross validation, it would be necessary to use further model interpretation methods and understand differences between the models (e.g. access which predictors are given which weight). However, the metrics indicate that environmental CV can be a good alternative to spatial CV when trying to access how well a model can be transferred to conditions outside the training data. 
